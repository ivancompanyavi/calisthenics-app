...
**Pattern slots** let a workout say "vertical pull" instead of naming an exercise. At session time the slot picks the version of that pattern the athlete is actually working — the unlocked progression they most recently trained, explicitly adopted, or manually unlocked — falling back to the easiest unlocked one when the chain is untouched. Unlocking a harder line never moves the athlete onto it automatically: it appears as a "step up?" card (Home and workout preview) that they accept or snooze. Entry gates judge *current form* — only PRs inside the recent evidence window count — so a line earned before a layoff re-locks instead of the program chasing peak-day numbers forever. The week's structure never changes; the difficulty underneath it does.

**Locked-slot substitution** answers the question the entry gate can't: "this is locked, so what do I do today?" A locked slot is replaced by the work that unlocks it — Back Lever gated behind a 45-second dead hang becomes a 45-second dead hang — so the gate clears itself simply by training. If that unlock work is already elsewhere in the same session, the slot falls back to the hardest unlocked exercise in the same pattern; if there's nothing distinct left to offer, the slot is dropped. An *unlocked* optional line the athlete never opted into gets the same treatment in spirit: the slot maintains its gate evidence (a dead hang, not a German Hang nobody asked for) until they accept the step-up card. A movement is never prescribed twice in one session.
